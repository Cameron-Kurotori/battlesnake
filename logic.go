...
func calculateSnakeWeight(dir sdk.Direction, me sdk.Battlesnake, board sdk.Board) float64 {
	...
	for _, snake := range board.OtherSnakes(me.ID) {
		for _, piece := range snake.Body {
			if piece.InDirectionOf(me.Head, dir) {
				directionalDistances = append(directionalDistances, piece.Manhattan(me.Head))
			}
			totalSnakeDistances += piece.Manhattan(me.Head)
		}
	}
...
func (m heuristicMover) Move(state sdk.GameState) sdk.BattlesnakeMoveResponse {
	...
	for _, dir := range state.You.Moves(logger) {
		...
		foodExponent := 1.0
		if state.You.Health < 60 || avgLenDiff > -2.5 {
			foodExponent = 3 * (avgLenDiff + 5) / (float64(state.You.Health))
